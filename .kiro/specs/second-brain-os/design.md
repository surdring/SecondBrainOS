...
- **职责边界**：
  - WeKnora：文档解析、分块、混合检索、可选 GraphRAG 增强
  - SBO：检索编排、时间权重重排、证据模型统一、权限隔离与审计
- **查询能力**：支持文档级/片段级检索，必要时启用 GraphRAG 增强以支持更强的关系推理
...
3. 统一排序与裁剪：按时间优先与置信度阈值过滤，返回 top-N（建议 3~8）。
4. 返回 `evidence[]`：每条证据包含类型、文本、发生时间、来源、置信度与引用信息（用于跳转/审计）。
